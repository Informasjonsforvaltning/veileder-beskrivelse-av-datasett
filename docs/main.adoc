include::locale/attributes.adoc[]
= Veileder for beskrivelse av datasett
:description:
:doctype: book
:docinfo:
:icons: font
:toc: left
:toclevels: 3
:sectanchors:
:imagesdir: images
:title-logo-image: image:digitaliseringsdirektoratet.png[pdfwidth=30vw]

image::digitaliseringsdirektoratet.png[width=50%, pdfwidth=30vw]

include::./shared/download.adoc[]

NOTE: *Innmelding av feil og mangler:* +
Dersom du finner feil eller mangler i dokumentet, ber vi om at dette meldes inn på https://github.com/Informasjonsforvaltning/dcat-ap-no-veileder/issues[Github Issues]. Dersom du ikke allerede har bruker på Github kan du opprette bruker gratis.


*Status*: [yellow-background]#under utarbeidelse# +
*Versjon*: 2.0 +
*Publisert*: [yellow-background]#under utarbeidelse# +
*Gjeldende versjon*: https://data.norge.no/guide/veileder-beskrivelse-av-datasett/ +
*Forrige versjon*: https://data.norge.no/guide/veileder-beskrivelse-av-datasett/v1 [yellow-background]#(kommer)# +
*Redaktørens utkast*: https://informasjonsforvaltning.github.io/veileder-beskrivelse-av-datasett/



// :leveloffset: +1

// Innhold her.
include::Innledning.adoc[]
include::Hva_er_et_datasett.adoc[]
include::Datasett.adoc[]
include::Enhet.adoc[]
include::Kontaktpunkt.adoc[]
include::Distribusjon.adoc[]
include::Datakatalog.adoc[]
include::Referanser.adoc[]



include::./shared/stats.adoc[]
